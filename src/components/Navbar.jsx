...
const NavLink = ({ to, label }) => {
  ...
  return (
    <Link to={to} className="relative text-sm font-medium text-slate-300 hover:text-white transition-colors py-1">
      {label}
      <motion.span
...
const Logo = () => (
  <Link to="/" className="flex items-center gap-2 group">
    <motion.div
      ...
      className="w-8 h-8 rounded-lg bg-electric/15 border border-electric/40 flex items-center justify-center"
    >
      ...
      <nav className="max-w-6xl mx-auto flex items-center justify-between px-6 py-4">
        ...
        <div className="hidden md:flex items-center gap-4">
          {user ? (
            ...
            <>
              <Link to="/login" className="text-sm font-medium text-slate-300 hover:text-electric transition-colors">
                Login
              </Link>
              <Link
                to="/register"
        ...
        {menuOpen && (
          ...
          >
            <div className="flex flex-col gap-1 px-6 py-4">
              ...
              {user ? (
                <>
                  <Link to="/profile" onClick={() => setMenuOpen(false)} className="py-2 text-sm text-slate-300">
                    {user.name}
                  </Link>
                  <button onClick={handleLogout} className="py-2 text-left text-sm text-electric">
                    Logout
                  </button>
                </>
              ) : (
                <>
                  <Link to="/login" onClick={() => setMenuOpen(false)} className="py-2 text-sm text-slate-300">
                    Login
                  </Link>
                  <Link to="/register" onClick={() => setMenuOpen(false)} className="py-2 text-sm text-electric">
                    Register
                  </Link>
                </>
              )}
